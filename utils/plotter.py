...
def draw_total_momentum_with_gauss(src: Source, gp: GaussParams, out_dir: Path, use_cb: bool = False) -> None:
    ...
    # 4) estimate local background AS A LINE and anchor it to the left edge
    # ---------------------------------------------------------------------

    def _percentile(vs, q):
        if not vs: return 0.0
        ss = sorted(vs)
        k = int(round((len(ss)-1) * max(0.0, min(100.0, q)) / 100.0))
        return ss[k]

    # (a) collect low-mass bins well before the peak, to avoid the shoulder
    low_x_max = peak_mu - 1.6 * peak_sigma
    if low_x_max <= xmin + 0.2:
        low_x_max = xmin + 0.2

    xs_low, ys_low = [], []
    for b in range(1, nbins + 1):
        xb = hist.GetBinCenter(b)
        if xmin <= xb <= low_x_max:
            yb = hist.GetBinContent(b)
            if yb > 0:
                xs_low.append(xb)
                ys_low.append(yb)

    # (b) build a *slightly high* pedestal from quantiles
    #     -> not the minimum, not the max, something in between
    ped0 = 0.0
    slope0 = 0.0

    if len(ys_low) >= 5:
        ys_sorted = sorted(ys_low)
        nlow = len(ys_sorted)
        ...
        q35 = q(0.35)
        q65 = q(0.65)
        # a bit higher than before → lifts CB tail
        ped0 = 0.35 * q35 + 0.65 * q65

        # small linear fit on ALL low-mass points (not only envelope)
        n   = float(len(xs_low))
        sx  = sum(xs_low); sy = sum(ys_low)
        sxx = sum(x*x for x in xs_low)
        sxy = sum(x*y for x, y in zip(xs_low, ys_low))
        denom = n * sxx - sx * sx
        if abs(denom) > 1e-6:
            m_data = (n * sxy - sx * sy) / denom
            slope0 = m_data
        else:
            slope0 = 0.0
    else:
        # fallback: take what you see at the very left
        left_edge_vals = [hist.GetBinContent(b)
                          for b in range(1, min(6, nbins) + 1)]
        ped0 = max(0.0, sum(left_edge_vals) / max(len(left_edge_vals), 1))
        slope0 = 0.0

    # (c) final safety clamps on slope:
    #     we don't want the stupid diagonal -> keep slope tiny
    visible_span = max(peak_mu - xmin, 0.5)
    slope_unit = max(0.5, ped0 / visible_span)   # "reasonable" slope scale
    neg_cap = -0.15 * slope_unit
    pos_cap = +0.35 * slope_unit
    if slope0 < neg_cap: slope0 = neg_cap
    if slope0 > pos_cap: slope0 = pos_cap


    # common amplitude guess for both Gauss and CB
    amp0 = max(peak_height - ped0, 1.0)

    if not use_cb:
        # --------------------------------------------------------------
        # 5A) Gaussian + linear background around the peak
        #     [0] amp, [1] mean, [2] sigma, [3] pedestal, [4] slope
        # --------------------------------------------------------------
        gfun = r.TF1(
            "gaussfit",
        ...
        # slope
        max_slope = max(0.8, 1.2 * abs(slope0))
        cb_fit.SetParLimits(6, -max_slope, +max_slope)
        ...
        # --------------------------------------------------------------
        # 5B) Crystal Ball (low-side) + linear background, CMSSW-safe
        #     p0 amp, p1 mean, p2 sigma, p3 alpha, p4 n, p5 pedestal, p6 slope
        # --------------------------------------------------------------
        cb_name = f"cbfit_{src.det}_{uuid.uuid4().hex}"

        cb_formula = (
            "(((x-[1])/[2]) > -[3] ? "
            "([0]*exp(-0.5*((x-[1])/[2])^2)) : "
            "([0]*pow([4]/[3],[4])*exp(-0.5*[3]*[3])"
            "*pow([4]/[3]-[3]-((x-[1])/[2]), -[4])) )"
            " + [5] + [6]*(x-[1])"
        )

        # 1) fit only in the narrow window -> this keeps robustness
        cb_fit = r.TF1(cb_name, cb_formula, fit_min, fit_max)

        # Seed CB tail ONLY from what’s visible now (current hist window)
        probs = array('d', [0.07])   # probe a slightly deeper tail (7%)
        quans = array('d', [0.0])
        try:
            hist.GetQuantiles(1, quans, probs)
            xq = float(quans[0])
        except Exception:
            xq = peak_mu - 1.2 * peak_sigma  # fallback inside the window
        alpha0 = max(0.8, min(3.0, (peak_mu - xq) / max(peak_sigma, 1e-6)))
        n0 = 4.0  # robust default

        cb_fit.SetParameter(0, amp0)          # amp
        cb_fit.SetParameter(1, peak_mu)       # mean
        cb_fit.SetParameter(2, peak_sigma)    # sigma
        cb_fit.SetParameter(3, alpha0)        # alpha from visible tail
        cb_fit.SetParameter(4, n0)            # n
        cb_fit.SetParameter(5, ped0 * 1.12)   # seed 12% higher to match shoulder
        cb_fit.SetParameter(6, slope0)        # slope

        # limits
        cb_fit.SetParLimits(1,
                            peak_mu - 0.25 * peak_sigma,
                            peak_mu + 0.25 * peak_sigma)
        cb_fit.SetParLimits(2,
                            0.35 * peak_sigma,
                            2.00 * peak_sigma)
        cb_fit.SetParLimits(3, 0.8, 4.0)   # alpha
        cb_fit.SetParLimits(4, 2.0, 10.)   # n
        cb_fit.SetParLimits(5, 0.0, 2.0 * max(ped0, 1.0))
        visible_span = max(peak_mu - xmin, 0.5)
        slope_unit = max(0.5, ped0 / visible_span)
        neg_cap = -0.25 * slope_unit
        pos_cap = +0.80 * slope_unit
        cb_fit.SetParLimits(6, neg_cap, pos_cap)


        if hist.GetEntries() > 0:
            hist.Fit(cb_fit, "QR", "", fit_min, fit_max)
            fit_mu = cb_fit.GetParameter(1)
            # keep the peak locked if it tries to run away
            if abs(fit_mu - peak_mu) > 0.30 * peak_sigma:
                cb_fit.SetParameter(1, peak_mu)
                cb_fit.SetParameter(2, peak_sigma)

        # 2) Build a DRAWING version on the FULL x-range
        #    (no scaling, just extend the fitted CB)
        xlo = hist.GetXaxis().GetXmin()
        xhi = hist.GetXaxis().GetXmax()

        cb_full = r.TF1(cb_name + "_full", cb_formula, xlo, xhi)

        # copy fitted params 0..6
        for ipar in range(7):
            val = cb_fit.GetParameter(ipar)
            cb_full.SetParameter(ipar, val)
            cb_full.FixParameter(ipar, val)

        cb_full.SetLineColor(r.kRed)
        cb_full.SetLineWidth(2)

        # reuse this for legend/draw below
        gfun = cb_full
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--crystalball", action="store_true",
                   help="Use Crystal Ball (CB + linear bkg) instead of Gaussian")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    for det in tqdm(detectors, desc="Detectors"):
        ...
        try:
            draw_total_momentum_with_gauss(src, gp, det_dir, use_cb=args.crystalball)
